...
[![Security Rating](https://sonarcloud.io/api/project_badges/measure?project=DEFRA_aice-triage-frontend&metric=security_rating)](https://sonarcloud.io/summary/new_code?id=DEFRA_aice-triage-frontend)
...
> compatibility fEx: `docker build --platform=linux/arm64 --no-cache --tag aice-triage-frontend`

Build:

```bash
docker build --target development --no-cache --tag aice-triage-frontend:development .
```

Run:

```bash
docker run -p 3000:3000 aice-triage-frontend:development
```
...
```bash
docker build --no-cache --tag aice-triage-frontend .
```

Run:

```bash
docker run -p 3000:3000 aice-triage-frontend
```
